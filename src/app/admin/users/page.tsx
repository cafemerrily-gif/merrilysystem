  ...
  return (
    <div className="min-h-screen pb-16" style={{ backgroundColor: bgColor, color: textColor }}>
      ...
      <main className="pt-20 max-w-2xl mx-auto px-4 py-6">
        <div className="space-y-0 border rounded-2xl overflow-hidden" style={{ borderColor }}>
          {members.map((member, index) => (
            <div 
              key={member.user_id} 
              className={`flex items-center gap-3 p-4 ${index !== members.length - 1 ? 'border-b' : ''}`}
              style={{ borderColor }}
            >
              {/* アバター */}
              <div className="relative w-12 h-12 rounded-full overflow-hidden" style={{ backgroundColor: isDark ? '#262626' : '#efefef' }}>
                {member.avatar_url ? (
                  <Image src={member.avatar_url} alt={member.display_name} fill className="object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <svg className="w-7 h-7" fill="none" stroke={mutedColor} viewBox="0 0 24 24">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs" style={{ color: mutedColor }}>
                  {member.role === 'admin' ? '管理者' : member.role === 'member' ? 'メンバー' : '一般'}
                </p>
              </div>
